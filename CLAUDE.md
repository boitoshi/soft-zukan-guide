...
#### パフォーマンス考慮
- **v-for のkey**: 一意のkey必須（**pokemon.name を使用** ※pokemon.id は地域図鑑番号で一意ではない）
...
### モデル使い分け
- **Opus**: 検討・考察・プラン作成・レビュー・方針決め・バグ調査
- **Sonnet（Task サブエージェント）**: コードの実装・ファイル編集作業
- 難しい判断が必要な場面は Opus が担当し、定型的なコード書き換えは Sonnet に委任する

### 実装後の確認（必須）
- コードを書いたら**必ず** `npx vue-tsc --noEmit` と `npx vite build` を実行する
- 書いて終わりではなく、**変更したファイルを再読して意図通りか確認**する
- フィルターやイベントハンドラーなど**ユーザー操作に関わる部分**は特に注意して動作を検証する

### Vue 3 テンプレートのイベントハンドラー
- **カリー化関数をテンプレートで使わない**: `@change="handler('arg')"` で handler が関数を返す場合、内側の関数は実行されない（Vue がインライン式として評価するため）
- **正しい書き方**: `@change="updateFilter('key', ($event.target as HTMLSelectElement).value)"` のように直接処理を書く
- これは Vue 3 のテンプレートコンパイラの仕様。`@event="expr"` は `$event => { expr }` にコンパイルされ、返り値は無視される

### ポケモンデータの一意性（重要）
- **pokemon.id は地域図鑑番号であり、ゲーム内で一意ではない**（例: ガラル図鑑で id:"001" が4匹存在する）
- **v-for の :key は必ず `pokemon.name` を使用する**（pokemon.id は絶対にダメ）
- ポケモンの特定・トグル・検索は全て **ポケモン名（name）ベース** で行う
- caught 状態は `useGlobalProgress` から導出する。pokemon オブジェクトの `caught` プロパティを直接変更しない

### UI/UX の原則
- **スクロール量を最小化**: ユーザーが目的のコンテンツ（ポケモンリストなど）に素早く到達できるようにする
- **ラベルは曖昧にしない**: 「両バージョン」と「全て表示」のように意味が紛らわしい表現を避ける
- **重複する UI 要素に注意**: コンポーネントのデフォルト値とデータの両方に同じオプションがあると重複表示になる
- コンパクトさを優先し、装飾的なヘッダー・グラデーション・大きなパディングは避ける

### データ整合性
- JSON データファイルのオプションとコンポーネント内のハードコードオプションが重複しないようにする
- バージョンフィルターのデフォルト（空値="全て"）はコンポーネント側で1箇所だけ管理する
- 新しいゲームデータを追加する際は、既存のフィルター構造と一致させる

### 困ったら立ち止まる
- 実装に迷ったら**プランモードに入って検討**する
- 既存のコードパターンを確認してから実装する（例: useGlobalProgress のパターンが正解だった）
- 「動くかも」で書かず、**なぜ動くか理解してから書く**
...
**💡 最終更新**: 2026年2月 - 開発ルール追加（モデル使い分け・実装後確認・Vue3ハンドラー注意・ポケモンID一意性・UI/UX原則）
