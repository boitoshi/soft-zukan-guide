#!/usr/bin/env node
/**
 * ポケモン名 英語→日本語 一括変換スクリプト
 *
 * 対象: public/ 配下の図鑑データJSONファイル (XY, ORAS, SM, USUM)
 * 全国図鑑番号をキーにして日本語名に変換する
 */
...
// 全国図鑑番号 → 日本語名マッピング (Gen 1-7: #001-#807)
const JAPANESE_NAMES = {
  ...
  322: 'ドンメル', 323: 'バクーダ', 324: 'コータス', 325: 'バネブー', 326: 'ブーピッグ',
...
const TARGET_FILES = [
  'xy_zukan_data.json',
  'oras_zukan_data.json',
  'sm_zukan_data.json',
  'usum_zukan_data.json',
];
...
function convertFile(filename) {
  ...
  for (const pokemon of data.pokemon) {
    const id = parseInt(pokemon.id, 10);
    const jaName = JAPANESE_NAMES[id];

    if (jaName) {
      if (pokemon.name !== jaName) {
        pokemon.name = jaName;
        converted++;
      }
    } else {
      console.log(`  ⚠️  ID ${pokemon.id} (${pokemon.name}) の日本語名が見つかりません`);
      notFound++;
    }
  }
...
for (const filename of TARGET_FILES) {
  ...
  const { converted, notFound } = convertFile(filename);
  totalConverted += converted;
  totalNotFound += notFound;
  console.log(`  ✅ ${converted}匹を日本語名に変換`);
  if (notFound > 0) {
    console.log(`  ⚠️  ${notFound}匹のマッピングが見つかりません`);
  }
  console.log();
}

console.log('━'.repeat(40));
console.log(`🎉 変換完了！合計 ${totalConverted}匹を日本語名に変換`);
if (totalNotFound > 0) {
  console.log(`⚠️  ${totalNotFound}匹のマッピングが見つかりませんでした`);
}
