...
/**
 * 🔧 剣盾図鑑.txt → JSON変換スクリプト
 * 
 * タブ区切りの剣盾図鑑データをJSON形式に変換します
 * - ガラル図鑑
 * - ヨロイ図鑑  
 * - カンムリ図鑑
 */
...
console.log('🗡️ 剣盾図鑑データの変換を開始...');

// 入力ファイル確認
const inputFile = '../data/raw/剣盾図鑑.txt';
if (!fs.existsSync(inputFile)) {
    console.error('❌ data/raw/剣盾図鑑.txt が見つかりません');
    process.exit(1);
}
...
for (let i = 1; i < lines.length; i++) {
    ...
    // ヨロイ図鑑 (列2,3)
    if (parts[2] && parts[3] && parts[2] !== parts[1]) {
        regions.push('armor');
        pokedex_numbers.armor = parts[2].padStart(3, '0');
    }
    
    // カンムリ図鑑 (列4,5)
    if (parts[4] && parts[5] && parts[4] !== parts[3]) {
        regions.push('crown');
    ...
    if (pokemonMap.has(pokemonName)) {
        ...
        // 新しいポケモンを登録
        const pokemonId = `pokemon_${i.toString().padStart(4, '0')}`;
