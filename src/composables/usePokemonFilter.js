...
export function usePokemonFilter(zukanData, selectedGame) {
    // フィルター状態
    const filters = ref({
        region: '',
        status: '',
    ...
    const filteredPokemon = computed(() => {
        ...
        if (filters.value.scarlet_violet && filters.value.scarlet_violet !== 'all') {
            filtered = filtered.filter((p) => p.version_info?.scarlet_violet?.availability === filters.value.scarlet_violet);
        } // バージョンフィルター - ソード・シールド
        if (filters.value.sword_shield && filters.value.sword_shield !== 'all') {
            filtered = filtered.filter((p) => p.version_info?.sword_shield?.availability === filters.value.sword_shield);
        }
        return filtered;
    });
    // フィルターリセット
